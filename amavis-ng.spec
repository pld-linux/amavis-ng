...
if [ -n "`getgid amavis`" ]; then
	if [ "`getgid amavis`" != "116" ]; then
		if [ "$1" = 2 ]; then
			echo "changing amavis group to GID=116"
			/usr/sbin/groupmod -g 116 amavis
		else
			echo "Warning: group amavis doesn't have gid=116. Correct this before installing amavis" 1>&2
			exit 1
		fi
	fi
else
